...
void CSolution::vCopySizes(const CSolution& pcOther)
{
	vDelete();
	i_suppliers_count = pcOther.i_suppliers_count;
	i_factories_count = pcOther.i_factories_count;
	i_warehouses_count = pcOther.i_warehouses_count;
	i_sellers_count = pcOther.i_sellers_count;
	i_size = i_suppliers_count * i_factories_count + i_factories_count * i_warehouses_count + i_warehouses_count * i_sellers_count;
	vInitMatrixes();
}
...
bool CSolution::bIsIdentical(const CSolution& pcOther)
{
	if (i_size != pcOther.i_size) return false;
	for (int i = 0; i < i_size; i++)
	{
		if (pd_solution->dGet(i) != pcOther.pd_solution->dGet(i)) return false;
	}
	return true;
}
